...
- **词库**——把模型老听错的词加进去。只有足够独特的拼写会被自动纠正：`TestFlight` 能从 "test flight" 救回来；`xAI` 从 "what's a ship" 救不回来——念起来几乎一样，加多少词库都没用。词库会随请求发给 OpenRouter，但它不转发给模型（已实测），纠错在本地做。
