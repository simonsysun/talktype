...
**App 免费（MIT）。** 你自己准备语音 API Key，用量按对应云厂商计费。没有 TalkType 账号、没有订阅、没有遥测。不是离线 / 本地引擎——声音会发到你选的那一家。
...
1. **下载**最新 macOS 安装包：  
   **[⬇ 最新 zip](https://github.com/simonsysun/talktype/releases/latest)**  
   解压 → 把 `TalkType.app` 拖进 **「应用程序」**。别下 2.x 的包——那是上一版本地引擎产品。

2. **第一次打开。** 这个构建没有苹果付费开发者证书，系统可能拦：  
   右键 TalkType → **打开** → **打开**。  
   还是不行就去 **系统设置 → 隐私与安全性** → **仍要打开**。
...
| 现象 | 处理 |
| --- | --- |
| 系统不让打开 | 右键 → 打开 → 打开。还是拦：系统设置 → 隐私与安全性 → 仍要打开 |
...
- **TalkType：** 免费。
- **语音 API：** 按你选的厂商计费，标价会变，以控制台为准。2026 年中的公开价，每天说几分钟几乎可以忽略：
  - 豆包流式语音识别 2.0 大约 **1 元 / 音频小时**
  - Grok REST 文件识别 **$0.10 / 音频小时**（如果以后做流式，是 $0.20）
