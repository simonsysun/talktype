...
语音识别用的是豆包（火山引擎）大模型录音文件识别极速版。它原生的标点、口语数字转写（「百分之九十五」→「95%」）和语义顺滑都打开了；后面没有第二个 LLM 改写你的话。
...
3. **填 API Key。** 第一次启动会自己弹出来。去豆包语音新版控制台 ▸ **API Key 管理**拿一个项目 Key——不是「访问控制」里的「API 访问密钥」。这个项目还要开通**录音文件识别大模型 极速版**。
...
- **提示"requested grant not found"？** 给这个项目开通**录音文件识别大模型 极速版**。
...
热词放在 `request.corpus.context`，而且它是一个 JSON **字符串**、不是嵌套对象。极速版已经实测接受；A/B 测试能把「Cloud Code」纠正成「Claude Code」。只在词库非空时才带上。
