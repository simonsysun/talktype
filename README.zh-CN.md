...
**用说的，代替打字。** 在 Mac 上任何地方按一下快捷键，把想说的说出来，松开手——文字就落在光标处。Slack、备忘录、邮件、终端……凡是能打字的地方，都能说。
...
| 什么 | 会离开你的 Mac 吗 |
|---|---|
| 音频（云端引擎） | 会——发给 OpenRouter。不留存、不用于训练 |
| 音频（本地引擎） | 不会 |
| 文字 → 润色（可选） | 只发文字给 Groq——从不发音频 |

没有账号、没有遥测、没有订阅。唯一的"账号"是你自己的 API key，存在 macOS 钥匙串里。
...
3. **填你的 OpenRouter key。** 设置 ▸ 云端引擎 ▸ 粘贴 key（在 [openrouter.ai/keys](https://openrouter.ai/keys) 获取；充几美元，正常用能撑一两个月）。云端听写立刻可用。
4. **可选：装本地引擎。** 设置 ▸ 本地引擎 ▸ **安装**——下载约 4 GB，只下一次。只有想离线用、或希望音频完全不出机器时才需要。
5. **可选但推荐：填一个 Groq key 做润色。** 设置 ▸ 云端润色 ▸ **添加 Groq key**（在 [console.groq.com/keys](https://console.groq.com/keys) 免费获取）。润色会去掉"呃""嗯"、补标点、理顺说到一半改口的话——只发文字，从不发音频。不填 key 也有本地规则清理兜底，效果不差。
...
- **润色：** [Groq](https://groq.com) 上的 qwen3.6-27b——约 0.3 秒，只发文字，并有严格护栏防止改写或翻译你说的话。
