...
- **边录边传，一个 provider。** 默认走豆包流式语音识别 2.0，松开快捷键时只需结束最后一帧；
  流式失败才用录音文件识别 2.0 极速版重试。两条路都显式打开原生标点、数字规范化和语义顺滑
  （`enable_punc` / `enable_itn` / `enable_ddc`），后面没有第二个模型改写。
- **词库直接作为热词传给豆包**（`request.corpus.context`），之后不再做任何本地纠正。所以一个词
  传了还是错，那就是豆包的真实水平。
- **一个 API Key。** 适配豆包语音新版控制台的 `X-Api-Key`，菜单栏只留一个安全输入框；服务没开通时
  直接告诉你去开「流式语音识别 2.0」和「录音文件识别 2.0」。
- **快捷键只交给 KeyboardShortcuts 管。** 删掉三套重复 fallback；用户清空快捷键后不会偷偷恢复
  `⌘⇧Space`。
- **长录音停止时动画不再卡住。** 音频硬件仍在主线程安全关闭，chunk 合并和重采样移到处理队列。
- **错误 Key 说人话。** 豆包用 HTTP 400 返回“app key not found”时，现在直接提示重填 API Key；
  明显的星号遮罩值和带空白的 Key 也不会被保存。
...
- v3 启动时清理上一代豆包双凭证、OpenRouter、Groq 和旧 provider 的钥匙串项。
